...
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=32,in_channels=64, num_classes=1, zero_head=False, vis=False):
        super(VisionTransformer, self).__init__()

        self.zero_head = zero_head
        self.classifier = config.classifier
        self.transformer = Transformer(config, img_size, in_channels, vis)
        self.decoder = DecoderCup(config, in_channels)
        self.config = config

    def forward(self, rgb):
        x, attn_weights, features = self.transformer(rgb)
